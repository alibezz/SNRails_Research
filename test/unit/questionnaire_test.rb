...
class QuestionnaireTest < Test::Unit::TestCase
  ...
  def setup
    ...
    @i1 = create_item(:type => 'Question', :survey_id => @survey.id, :info => "q1", :is_optional => false,                                     :html_type => Question.html_types.invert["single_selection"])
    @ivalue1 = create_item_value(:item_id => @i1.id)
    @i1.reload; @i1.save 

 
    @i2 = create_item(:type => 'Question', :survey_id => @survey.id, :info => "q2", :is_optional => false,                                     :html_type => Question.html_types.invert["single_selection"])
    @ivalue2 = create_item_value(:item_id => @i2.id)
    @i2.reload; @i2.save 


    @i3 = create_item(:type => 'Question', :survey_id => @survey.id, :info => "q3", :is_optional => true,                                     :html_type => Question.html_types.invert["single_selection"])
  ...
  def test_validate_obligatory_questions
    ...
    obligatory = @survey.questions.find_all { |q| q.is_optional == false}
  ...
  def test_associate_answers
    ...
    quest = create_questionnaire(:survey_id => @survey.id)
  ...
  def test_prepare_to_save
    quest = Questionnaire.new    
    assert quest.prepare_to_save({@i1.id.to_s => @ivalue1.id.to_s, @i2.id.to_s => @ivalue2.id.to_s, @i3.id.to_s => @ivalue3.id.to_s}, @survey.id)
    quest.save; quest.reload
    assert_equal false, quest.incomplete
    assert_equal quest.object_item_values.count, 3

    quest2 = Questionnaire.new    
    assert_equal false, quest2.prepare_to_save({@i1.id.to_s => @ivalue1.id.to_s, @i3.id.to_s => @ivalue3.id.to_s}, @survey.id)
  end 

  def test_validate_questions
    quest = create_questionnaire(:survey_id => @survey.id)
    quest.validate_questions({}, @survey.id)
    quest.reload
    assert_equal quest.errors.count, 1 
    
    quest2 = create_questionnaire(:survey_id => @survey.id)
    quest2.validate_questions({@i1.id.to_s => @ivalue1.id.to_s, @i2.id.to_s => @ivalue2.id.to_s, @i3.id.to_s => @ivalue3.id.to_s}, @survey.id)
    quest2.reload
