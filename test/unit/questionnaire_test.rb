...
class QuestionnaireTest < Test::Unit::TestCase
  ...
  def test_associate_answer
    ...
    quest.associate({"1" =>"1", "2"=>"2"})
    quest.reload
    assert_equal 2, quest.object_item_values.count

    quest.associate({"1" =>["1", "2"]})
    quest.reload
    assert_equal 3, quest.object_item_values.count
    ...
    quest3 = create_questionnaire
    quest3.associate({"7"=>["7"], "1"=>"1", "2"=>"3", "3"=>"5"})
    quest3.reload
    assert_equal 4, quest3.object_item_values.count
  end
